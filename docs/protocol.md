...
IPNS validates records only when resolving, but not when putting DHT value. This means it is possible to override a valid record with random bytes for an attacker, which slows down the resolution of a site. In conclusion, it doesn't make sense to use existing IPFS DHT for site resolution, for it causes many security problems and limits the features we could use. We need a specialized DHT for melotte in the future, which adds conditions to updating records, since structured network is usually better. To solve the withdrawal issue of a publickey, we introduce a `withdraw` flag in the record to announce that it shouldn't be updated anymore. This doesn't require the knowledge of that site, so peers won't need to download the whole management chain for DHT. Currently channel protocol is more suitable as we don't have a big network yet.
